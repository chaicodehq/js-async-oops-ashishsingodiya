...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    return new Promise((res, rej) => {
      ...
      if (this.#startups.some((s) => s.startup.name === startup.name)) return rej(new Error("Startup already admitted!"));
  ...
  async assignMentor(startupName, mentor) {
    return new Promise((res, rej) => {
      if (!this.#startups.find((s) => s.startup.name === startupName)) return rej(new Error("Startup not found!"));
  ...
  async conductDemo(startupName) {
    return new Promise((res, rej) => {
      ...
      if (!startup) return rej(new Error("Startup not found!"));
...
export async function runDemoDay(incubator) {
  let promises = [];
  for (const startup of incubator) {
    const promise = incubator.conductDemo(startup.name);
