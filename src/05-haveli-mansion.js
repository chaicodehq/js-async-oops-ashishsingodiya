...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    if (!allowedRoles.includes(role)) return { success: false, message: "Invalid role!" };

    const existing = this.#residents.some((r) => r.name === name);
    if (existing) return { success: false, message: "Already a resident!" };
